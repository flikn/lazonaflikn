...
def export_as_excel(modeladmin, request, queryset):
    ...
    response[
        "Content-Disposition"
    ] = "attachment; filename=%s.xls" % unicode(opts).replace(".", "_")
    try:
        ...
        v_field_names = [
            getattr(field, "verbose_name") or field.name for field in opts.fields
        ]
    v_field_names = map(
        lambda x: x.encode("utf-8") if x != "ID" else "Id", v_field_names
    )
